...
function makeQuestions(count, branch) { const pathIndex = branch === null ? 0 : Number.parseInt(branch.split('-')[1], 10); const shuffledScenes = shuffle(narrativeScenes); const scenes = shuffledScenes.map((_, index) => shuffledScenes[(index + pathIndex) % shuffledScenes.length]); return Array.from({length: count}, (_, index) => { const scene = scenes[index % scenes.length]; const firstPath = 'path-'+(index % 20); const secondPath = 'path-'+((index + 1) % 20); const choices = shuffle(scene.choices).map(([label, cost, response], choiceIndex) => [label, cost, response, branch === null ? (choiceIndex === 0 ? firstPath : secondPath) : branch]); return {time: String(7 + Math.floor(index / 3)).padStart(2,'0')+':'+String((index * 7) % 60).padStart(2,'0'), title:scene.title, copy:scene.copy, choices}; }); }
